# -*- coding: utf-8 -*-
"""
Created on Sat Oct 21 19:23:23 2023
...
def main():
    parser = argparse.ArgumentParser(description="Train ML models on 3-Slice canopy color percentage data to predict IDC scores")
    ...
    def drop_partial_slices(arg1):
        ...
        for i in range(size - 2):
            ...
            if arg1['plot'][i] == arg1['plot'][i + 1] and arg1['plot'][i + 1] == arg1['plot'][i + 2]:
          
                # printing the element as the 
                # conditions are satisfied 
                ID = arg1['ID'][i].split('_slice')[0]
                keep_list.append(ID)
                
                
          
        '''
        if arg1['ID'] is not in 'keep_list' then drop it
        '''
        for i in range(0,arg1.shape[0]):
            ID = arg1['ID'][i].split('_slice')[0]
            if ID not in keep_list:
                arg1.drop(index=i,inplace=True)
    ...
    def add_hierarchical_results(arg1, arg2, arg3):
        ...
        SMOTE_dict = {'SVMSMOTE': 'sampling_strategy = over_sampling_dict,random_state = %d' % (42)}
    ...
    def smote_classifier_m0(arg1, arg2, arg3, arg4):
        ...
        for key_1 in SMOTE_dict.keys():
            ...
            #save dataframe as png
            #df_styled = df_scores2.style.background_gradient()
            #dfi.export(df_styled,'fullcanopy_m0_%s_EXB.png' % (key_1), table_conversion = 'matplotlib')
    ...
    def smote_classifier_m1(arg1, arg2, arg3, arg4):
        ...
        for key_1 in SMOTE_dict.keys():
            ...
            #save dataframe as png
            #df_styled = df_scores2.style.background_gradient()
            #dfi.export(df_styled,'fullcanopy_m1_%s_EXB.png' % (key_1), table_conversion = 'matplotlib')
    ...
    def smote_classifier_m2(arg1, arg2, arg3, arg4):
        ...
        for key_1 in SMOTE_dict.keys():
            ...
            for key in modelDict.keys():
                ...
                # define pipeline
                #over = SMOTE(sampling_strategy=over_sampling_dict,random_state = 42)
                over = eval('%s()' % (key_1))
            ...
            #save dataframe as png
            #df_styled = df_scores2.style.background_gradient()
            #dfi.export(df_styled,'fullcanopy_m2_%s_EXB.png' % (key_1), table_conversion = 'matplotlib')
    ...
    for i in range(len(rdm_seed_lst)):
        ...
        for i in range(0,len(mbx[2])):
            ...
            elif m0[1].predict(mbx[2])[i] == 2:   #idc score is 3
                pred_list.append(3)
            elif m0[1].predict(mbx[2])[i] ==3: #idc score is 4 or 5
                pred = m2[1].predict(mbx[2])[i]
                pred_list.append(pred)
